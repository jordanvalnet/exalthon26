...
- Structure : une `<section>` couverture (sur-titre `design.eyebrow`, titre H1 du narratif, tuiles `design.kpis` prises dans
  ...
  une `<section>` finale (la section `deck.finale` du narratif, sinon les faits), une `<section>` Sources qui liste les citations
  en notes (sauf profil enfant : aucune note, aucune page Sources).
...
3. `bun run pdf <cache> <profil>`, puis ouvre le HTML et vérifie à l'œil : toutes les sections, tous les graphiques, aucun `[facts:…]` brut,
   les tuiles de couverture, l'exergue de chaque page, la finale, le nombre de pages du profil.
