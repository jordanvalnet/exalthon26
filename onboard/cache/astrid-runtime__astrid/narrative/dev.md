...
Licence relevée par GitHub : Apache-2.0 [facts:repo.license] ; le README et les fichiers `LICENSE-APACHE` / `LICENSE-MIT` disent « MIT OR Apache-2.0 » [src:readme.md] [facts:tree.11.path] [facts:tree.12.path]. Donnée non relevée : la répartition des langages ; à vérifier sur https://github.com/astrid-runtime/astrid. Le manifeste est un workspace Cargo [facts:deps.manifest].
...
Deux curiosités : `wit` est un fichier de 0 octet à la racine [facts:tree.27.role] et `.gitmodules` déclare des sous-modules [facts:tree.6.role] ; leur cible n'est pas relevée, à vérifier dans https://github.com/astrid-runtime/astrid/blob/main/.gitmodules.
...
2. `crates/astrid-cli/src/cli.rs` [facts:entrypoints.2.path] : définitions clap des sous-commandes (`agent`, `capsule`, `quota`, `status`, `start`, `stop`, `ps`, `top`) ; déduit du `mod cli` de main.rs, fichier non lu [facts:entrypoints.2.why].
...
Données non relevées : le corps de `astrid_daemon::run()` et le contenu de `crates/astrid-kernel/src/lib.rs` ; à vérifier dans https://github.com/astrid-runtime/astrid/tree/main/crates/astrid-daemon/src et https://github.com/astrid-runtime/astrid/tree/main/crates/astrid-kernel/src.
...
Données non relevées : les prérequis système (FUSE, FSKit, WinFsp) des tests de stockage et le temps de build ; à vérifier dans `scripts/` [facts:tree.26.role] et https://github.com/astrid-runtime/astrid/blob/main/.github/workflows/ci.yml.
...
Conséquence pratique : un seul relecteur pour vos PR ; 24 PR ouvertes [facts:pulls.open] pour 188 fusionnées en 30 jours [facts:pulls.merged_30d]. Donnée non relevée : combien attendent une revue ; à vérifier dans https://github.com/astrid-runtime/astrid/pulls.
...
Donnée non relevée : l'assignation éventuelle de ces issues ; à vérifier sur chaque URL avant de commenter pour la réclamer.
